...
class Analyze
  ...
  def self.count_correct_colors(secret_code, guess_code)
  ...
  def self.transform_code_array_into_hash(ary)
  ...
  def self.create_difference_hash(secret, guess)
  ...
  def self.create_new_hash_with_correct_colors(hash, colors_ary)
    hash.delete_if { |k, _v| k unless colors_ary.include? k }
  end

  # Count the number of correctly postionned colors
  def self.count_correct_positions(guess_code, secret_code)
    correct_positions = 0
    (0..3).each do |i|
